...
/*
    ...
    It contains the triggers for the final RPS Project
...
BEGIN
    ...
    ---------****** Trigger function for tbl_players ******---------

    CREATE OR REPLACE FUNCTION trg_players_block_delete_and_update()
    RETURNS TRIGGER AS $GO$
    BEGIN
        RAISE EXCEPTION 'Deleting or updating rows in tbl_players is not allowed';
        RETURN NULL;
    END;
    $GO$ LANGUAGE plpgsql;

    --------- Trigger to block DELETE and UPDATE ---------
    CREATE TRIGGER trg_players_block_delete_and_update
    BEFORE DELETE OR UPDATE ON tbl_players
    FOR EACH ROW
        EXECUTE FUNCTION trg_players_block_delete_and_update();
    ...
    ---------****** Trigger function for tbl_games ******---------

    CREATE OR REPLACE FUNCTION trg_games_block_delete_and_update()
    RETURNS TRIGGER AS $GO$
    BEGIN
        RAISE EXCEPTION 'Deleting or updating rows in tbl_games is not allowed';
        RETURN NULL;
    END;
    $GO$ LANGUAGE plpgsql;

    --------- Trigger to block DELETE and UPDATE ---------
    CREATE TRIGGER trg_games_block_delete_and_update
    BEFORE DELETE OR UPDATE ON tbl_games
    FOR EACH ROW
        EXECUTE FUNCTION trg_games_block_delete_and_update();
    ...
    ---------****** Trigger function for tbl_rounds ******---------

    CREATE OR REPLACE FUNCTION trg_rounds_block_delete_and_update()
    RETURNS TRIGGER AS $GO$
    BEGIN
        RAISE EXCEPTION 'Deleting or updating rows in tbl_rounds is not allowed';
        RETURN NULL;
    END;
    $GO$ LANGUAGE plpgsql;

    --------- Trigger to block DELETE and UPDATE ---------
    CREATE TRIGGER trg_rounds_block_delete_and_update
    BEFORE DELETE OR UPDATE ON tbl_rounds
    FOR EACH ROW
        EXECUTE FUNCTION trg_rounds_block_delete_and_update();
